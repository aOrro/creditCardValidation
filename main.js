...
let validateCardNumber = cardNumberArray => {
 ...
 //2nd- Iterating through the array, multiplying the numbers in (now) even indexes and substracting 9 if neccesary
  ...
  //3rd- Sum all the items in newArray
...
let idInvalidCardCompanies = arrayPassed => {

  //1st- Finding the invalid cards in the array passed 
  let invalidCardsList = findInvalidCards(arrayPassed);
  ...
  //3rd- Iterating through arrayPassed to find out the issuer's id for each card number
  for (x = 0; x < invalidCardsList.length; x++) {
    let firstDigit = invalidCardsList[x] [0];
...
//(STEP5) ---> Converting INVALID card number into VALID ones
let conversionToValid = invalidCard => {
  ...
  //3rd- Sum all the final digits of invalidCardCopy
...
console.log(invalid1); // Compare with conversionToValid
console.log(validateCardNumber(valid1));
console.log(validateCardNumber(invalid1));
console.log(findInvalidCards(batch));
console.log(idInvalidCardCompanies(batch));
console.log(convertStringToArray('4539712889515179'));
console.log(conversionToValid(invalid1)); */
